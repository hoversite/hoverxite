# Roadmap (public)

Rough order. Dates intentionally loose.

## Now

- [x] Public repo + open-core boundary
- [x] Landing shell
- [ ] Import URL → project files on disk
- [ ] Minimal agent edit loop (BYOK)
- [ ] Local preview of the project
- [ ] Export to git

## Next

- [ ] Better clone fidelity (layout, assets)
- [ ] Agent skills library (public markdown)
- [ ] Self-host docker-compose pack
- [ ] Hosted cloud waitlist / link (private product)

## Later (may open more)

- [ ] Simplified single-site publish helpers
- [ ] Optional self-host control plane template
- [ ] Multi-user self-host (still not multi-tenant edge)
